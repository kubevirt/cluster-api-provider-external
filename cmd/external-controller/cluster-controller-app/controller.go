...
import (
	...
	"github.com/kubevirt/cluster-api-provider-external/cmd/external-controller/cluster-controller-app/options"
...
const (
	gceClusterControllerName = "ext-cluster-controller"
)
...
func StartClusterController(server *options.ClusterControllerServer, shutdown <-chan struct{}) {
	...
	params := external.ClusterActuatorParams{
		ClusterClient: client.ClusterV1alpha1().Clusters(corev1.NamespaceDefault),
	}
	actuator, err := external.NewClusterActuator(params)
	if err != nil {
		glog.Fatalf("Could not create cluster actuator: %v", err)
	}
