...
import (
	...
	"github.com/kubevirt/cluster-api-provider-external/cmd/external-controller/machine-controller-app/options"
)
...
func StartMachineController(kClient *kubernetes.Clientset, server *options.MachineControllerServer, recorder record.EventRecorder, shutdown <-chan struct{}) {
	...
	actuator, err := external.NewMachineActuator(params)

	if err != nil {
		glog.Fatalf("Could not create external machine actuator: %v", err)
	}
